...
var CAP = function(args){
	...
	this.geomeLevel = 5;//默认几何划线级别
	this.noiseSigma = 5;//噪点级别
	this.noiseType = 2;//噪点类型
...
CAP.prototype._init = function(args){
	...
	if(typeof arguments[0] === 'object'){//如果传递了对象，则替换默认值
		...
		this.geomeLevel = obj.geomeLevel || this.geomeLevel;
		this.noiseSigma = obj.noiseSigma || this.noiseSigma;
		this.noiseType = obj.noiseType || this.noiseType;
		return;
	}
	//如果只传递了宽，高，间隔则替换默认值
...
CAP.prototype._call_create = function(text,len,j){//调用C++的CIMG库的create函数生成验证码文件
	...
	hrobj.create(text, filepath, len, that.width
	, that.height
	, that.offset
	, that.quality
	,isjpeg
